...
AI-powered peer code review. Connects to GitHub, analyses pull requests, and generates structured senior-engineer-level feedback using RAG + LLMs.
...
We use a PR-based workflow. No direct pushes to `main`.
...
chore: add celery to requirements
test: add unit tests for diff parser
docs: update setup instructions
refactor: extract diff parser to separate module
```
...
backend/
├── app/
│   ├── api/          Route handlers (auth, repos, reviews, webhook, feedback)
│   ├── models/       SQLAlchemy ORM models
│   ├── schemas/      Pydantic request/response schemas
│   ├── services/     Business logic (github, auth, diff parser, RAG, review, eval)
│   ├── llm/          LangChain chains, prompts, embeddings, output parser
│   ├── workers/      Celery tasks (review worker, index worker)
...
│   ├── components/   Reusable UI components (DiffViewer, ReviewComment, etc.)
...
- `docs/llm-pipeline.md` — RAG pipeline and prompt design
- `docs/decisions/` — architecture decision records (why we made key choices)
